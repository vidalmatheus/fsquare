...
function devhelp {
    ...
    echo -e ""
    echo -e "${LBLU}dknginx${RESTORE}           Runs a dockerized ${GREEN}nginx${RESTORE} on port 80 to 8000 (back), 3000 (front), 3001 (mocked front) and 3002 (metabase)"
    echo -e ""
    echo -e "${LBLU}pytest${RESTORE}            Runs python ${GREEN}tests${RESTORE}"
    echo -e ""
